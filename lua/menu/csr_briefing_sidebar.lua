...
-- lua/menu/csr_missions_menu.lua) so the briefing screen shows the same
...
-- the same reason the reminder does (see csr_briefing_reminder_input.lua):
...
--
-- This file is loaded AFTER csr_briefing_reminder_input.lua per mod.txt
...
-- this file doesn't depend on csr_missions_menu.lua's hook having already
...
if MissionBriefingGui and not _G._CSR_BRIEFING_SIDEBAR_HOOKED then
	...
	function MissionBriefingGui:_csr_build_sidebar()
		...
		-- gap (csr_missions_menu.lua: top = _title_bottom + sidebar_title_gap,
		...
		-- helpers (see csr_missions_menu.lua _setup for the lobby pair):
...
log("[CSR] csr_briefing_sidebar.lua loaded (MissionBriefingGui workspace sidebar + feature panels)")
