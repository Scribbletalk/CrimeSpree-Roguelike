...
-- Player items are no longer in active_modifiers(), so the old
-- CSR_FilterPlayerItemsFromActive wrapper is no longer needed.

-- Global variables
_G.CSR_SelectedDifficulty = _G.CSR_SelectedDifficulty or CSR_CurrentDifficulty or "overkill"
...
Hooks:PostHook(
	...
	"_setup_new_crime_spree",
	"CSR_AddDifficultySelection",
	function(self, text_w, text_h)
		...
		local title_text = "STARTING DIFFICULTY:"
		...
		local starting_level_text = "STARTING LEVEL:"
...
Hooks:PostHook(
	...
	function(self, button, x, y)
		...
		if CSR_DifficultyButtons then
			for _, btn in ipairs(CSR_DifficultyButtons) do
				if btn.selected and btn.panel and btn.panel:alive() then
					...
					-- If no seed exists yet (player picked a difficulty on a fresh install
					-- without ever starting a run), mint one now so line 2 is always on disk.
					-- Without this, quitting at rank 0 after selecting DW could leave the
					-- priority chain in _setup_global_from_mission_id falling through to the
					-- "overkill" fallback on next launch if the CS save blob also lost the field.
					if CSR_SaveSeed then
						if not CSR_CurrentSeed then
							CSR_CurrentSeed = os.time() + math.floor(math.random() * 1000000)
						end
		...
		if
			_G.CSR_LeftArrow
			and _G.CSR_LeftArrow.selected
			and _G.CSR_LeftArrow.panel
			and _G.CSR_LeftArrow.panel:alive()
		then
			-- Set held flag
		...
		if
			_G.CSR_RightArrow
			and _G.CSR_RightArrow.selected
			and _G.CSR_RightArrow.panel
			and _G.CSR_RightArrow.panel:alive()
		then
			-- Set held flag
