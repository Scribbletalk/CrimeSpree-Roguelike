...
local CB_K = C.crooked_badge_k or 0.05
...
if required == "lib/units/beings/player/playerdamage" then
	if PlayerDamage then
		...
		Hooks:PostHook(PlayerDamage, "init", "CSR_CrookedBadge_Init", function(self)
			if not CSR_ActiveBuffs or not CSR_ActiveBuffs.crooked_badge then
				return
			end
		...
		if not _G.CSR_CrookedBadge_DownTimeHooked then
			...
			_G.CSR_SafeOverride(PlayerDamage, "down_time", "Crooked Badge", original_down_time, function(self)
...
-- Local revive logic, runs against the local player_unit using the local
-- player's CSR_ActiveBuffs. Exposed as _G.CSR_CrookedBadge_OnAssaultEnd so
-- multiplayer_sync.lua can invoke it on clients when the host broadcasts
-- MSG.ASSAULT_END (the vanilla _begin_regroup_task only fires on the host,
-- so without an RPC clients with Crooked Badge would never get a revive).
local function csr_cb_apply_revive_for_local_player()
	if not CSR_ActiveBuffs or not CSR_ActiveBuffs.crooked_badge then
		return
	end
...
_G.CSR_CrookedBadge_OnAssaultEnd = csr_cb_apply_revive_for_local_player
...
if required == "lib/managers/group_ai_states/groupaistatebesiege" then
	if GroupAIStateBesiege then
		Hooks:PostHook(GroupAIStateBesiege, "_begin_regroup_task", "CSR_CrookedBadge_Regroup", function(self)
			-- Host-only entry point: vanilla assault state machine is server-authoritative
			if not Network or not Network:is_server() then
				return
			end

			-- Apply locally for the host
			csr_cb_apply_revive_for_local_player()

			-- Broadcast to clients so each one applies its own roll using its own stacks
			if LuaNetworking and CSR_MP and CSR_MP.MSG and CSR_MP.MSG.ASSAULT_END then
				LuaNetworking:SendToPeers(CSR_MP.MSG.ASSAULT_END, "1")
			end
		end)
	else
	end
