...
-- Formula: specials × (1 + 0.5 × stacks), normals × (0.5 ^ stacks) multiplicative
...
local SPECIAL_KEYWORDS = {
	...
	"medic", -- medic (also matches tank_medic, but tank catches that first)
	"captain", -- Winters himself
	"phalanx", -- Winters' shields: phalanx_minion, phalanx_vip
	"sniper",
	"shield",
	"marshal",
}
...
local function get_tweak_name(cop_damage)
	local ok, result = pcall(function()
		if not cop_damage._unit then
			return nil
		end
		if not alive(cop_damage._unit) then
			return nil
		end
		...
		if type(tt) == "table" then
			return tt._name or tt.name or nil
		end
		return nil
	end)
...
local function apply_equalizer(self, attack_data)
	if not CSR_ActiveBuffs or not CSR_ActiveBuffs.equalizer then
		return
	end
	...
	if is_special_unit(self) then
		attack_data.damage = attack_data.damage * (1 + (C.equalizer_bonus or 0.5) * stacks)
	else
		-- Multiplicative stacking: each stack multiplies by (1 - penalty)
		-- e.g. 2 stacks at 50%: damage × 0.5 × 0.5 = 25% (not 0%)
		attack_data.damage = math.max(1, attack_data.damage * (1 - (C.equalizer_penalty or 0.5)) ^ stacks)
	end
end
