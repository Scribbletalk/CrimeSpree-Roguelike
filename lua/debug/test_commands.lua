...
Hooks:Add("GameSetupUpdate", "CSR_TestCommandsUpdate", function(t, dt)
	...
	if Input:keyboard():pressed(Idstring("f9")) then
		...
		if managers.crime_spree and managers.crime_spree.enable_crime_spree_gamemode then
			-- Enable Crime Spree (safe call with pcall)
			print("[CSR Test] DEBUG: Calling enable_crime_spree_gamemode()...")
			...
			print("[CSR Test] ✓ enable_crime_spree_gamemode() successful")
	...
	if Input:keyboard():pressed(Idstring("f10")) then
		...
		if managers.crime_spree and managers.crime_spree._global then
			...
			-- Add to per-player item store (not _global.modifiers)
			if CSR_AddItem then
				for _, mod in ipairs(test_mods) do
					-- Extract prefix: "player_plush_shark_1" -> "player_plush_shark_"
					local prefix = mod.id:gsub("%d+$", "")
					CSR_AddItem(prefix, mod.level)
				end
			end
