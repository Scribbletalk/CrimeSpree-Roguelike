...
-- Registry-driven item lookup
local _registry = _G.CSR_ITEM_REGISTRY or {}
...
local function is_player_item(mod_id)
	...
	for _, item in ipairs(_registry) do
		if string.find(mod_id, item.id_prefix, 1, true) then
			return true
		end
	end
	return false
...
-- (no longer used — seed is fully deterministic per CS run state)
...
-- Function to get item type by id (registry-driven)
...
if CrimeSpreeManager then
	...
	function CrimeSpreeManager:server_active_modifiers()
		...
		for _, mod_data in ipairs(modifiers) do
			-- Skip dummy modifiers (always)
			if mod_data.id and string.find(mod_data.id, "csr_dummy", 1, true) then
				-- skip
				-- Skip player items (always) — they live in CSR_PlayerItems, not here
			elseif mod_data.id and is_player_item(mod_data.id) then
				-- skip
				-- Filter HP/DMG modifiers only when UI flag is enabled
		...
		-- Sort by level ascending — vanilla renders the list in reverse (#modifiers..1),
		-- so ascending here = visually highest rank at top, lowest at bottom.
		-- Virtual HP/DMG mod is pinned to the end of the array so it renders first (top).
		if CSR_FilterForUI then
			table.sort(filtered, function(a, b)
				local a_virtual = a.id == "csr_enemy_hp_damage_total"
				local b_virtual = b.id == "csr_enemy_hp_damage_total"
				if a_virtual then
					return false
				end
				if b_virtual then
	...
	-- NOTE: multiplayer_sync.lua also overrides get_modifier (loaded later via mod.txt).
	-- Call chain: multiplayer_sync -> this override -> vanilla.
	...
	-- Override active_modifiers — strip dummy modifiers.
	-- Player items are no longer in _global.modifiers (they live in CSR_PlayerItems).
	...
	function CrimeSpreeManager:active_modifiers()
		...
		-- Filter dummy modifiers and any leaked player items
		local filtered = {}
		for _, mod_data in ipairs(modifiers) do
			if mod_data.id and string.find(mod_data.id, "csr_dummy", 1, true) then
				-- skip dummy
			elseif mod_data.id and is_player_item(mod_data.id) then
				-- skip leaked player items
			else
				table.insert(filtered, mod_data)
			end
		end
	...
	-- Handles two cases that vanilla cannot:
	-- 1. csr_enemy_hp_damage_total — virtual combined HP/DMG modifier, not in tweakdata at all
	-- 2. player_* items — not in tweakdata, crash if vanilla tries pairs(data.data) on nil
	-- All other csr_* forced modifiers ARE in tweak_data.crime_spree.modifiers.forced with
	-- full IDs (e.g. "csr_civilian_guilt_20"), so vanilla handles them correctly via fallback.
	...
	function CrimeSpreeManager:make_modifier_description(id, ...)
		...
		if id == "csr_enemy_hp_damage_total" then
			...
			-- Break out the printer contribution so players can see what the
			-- cost is costing them (rank-based bonus vs printer-cost bonus).
			local printer_dmg = 0
			if _G.CSR_PrinterUses then
				...
				local per_use = (_G.CSR_ItemConstants or {}).printer_damage_taken_per_use or 0.004
				printer_dmg = uses * per_use
			end

			if printer_dmg > 0 then
				local rank_dmg = dmg_bonus - printer_dmg
				return string.format(
					"Enemy HP: +%.1f%%  |  Enemy Damage: +%.1f%% + %.1f%%",
					hp_bonus * 100,
					rank_dmg * 100,
					printer_dmg * 100
				)
		...
		-- Forced modifiers (csr_*) — try our lookup first, then fall back to vanilla.
		-- Our lookup resolves the desc_id path quickly.
		-- If anything fails, vanilla can handle it: modifiers.forced contains full IDs and
		-- all modifier classes have desc_id set, so vanilla won't crash or return empty text.
		if string.find(id, "^csr_") then
			...
			-- Secondary: search repeating_modifiers.forced
			...
			-- If our lookup found the data, resolve the description via desc_id
			if mod_data then
			...
			-- Fall back to vanilla — guard with pcall because vanilla crashes when
			-- get_modifier() returns nil or a class that is not in global scope
			local ok, result = pcall(original_make_modifier_description, self, id, ...)
	...
	-- On-the-fly modifier factory: builds a fresh modifier from the registry
	local _by_type = _G.CSR_ITEM_BY_TYPE or {}

	local function generate_new_modifier(item_type, next_id)
		local item = _by_type[item_type]
		if not item then
			return nil
		end

		return {
			id = item.id_prefix .. next_id,
			class = item.class,
			icon = item.icon,
			level = 0,
			data = {},
	...
	local function get_next_id(item_type)
		...
		-- Read from per-player item store instead of active_modifiers
		local my_items = CSR_GetLocalItems and CSR_GetLocalItems() or {}
		local max_id = 0

		for _, mod_data in ipairs(my_items) do
			if mod_data.id and string.find(mod_data.id, item.id_prefix, 1, true) then
	...
	local function is_contraband(item_type)
		local item = _by_type[item_type]
		return item and item.rarity == "contraband"
	end
	...
	local _mts_log_next = 0
	function CrimeSpreeManager:modifiers_to_select(table_name, add_repeating)
		if table_name == "loud" then
			-- Clients in MP: use host's rank (own spree_level is 0 from auto-create)
			local level
			local using_host_rank = false
			if _G.CSR_MP and CSR_MP.is_client and CSR_MP.is_client() and _G.CSR_MP_HostRank then
				level = _G.CSR_MP_HostRank
				using_host_rank = true
			else
				level = self:spree_level() or 0
			end
			local expected = math.floor(level / 20)

			-- Count items from per-player store (not _global.modifiers)
			local my_items = CSR_GetLocalItems and CSR_GetLocalItems() or {}
			local selected = #my_items

			-- Bonus drop items are NOT milestone picks, subtract them.
			-- Shop purchases (Gage's Services) are also extra — they stack on top
			-- of rank grants instead of filling the quota, so we subtract them
			-- from the active count the same way bonus drops are subtracted.
			-- Catchup grants on late join are simulated shop purchases — same exemption.
			local bonus_items = _G.CSR_BonusDropCount or 0
			local shop_items = 0
			local catchup_items = 0
			local pid = (_G.CSR_LocalPeerId and CSR_LocalPeerId()) or 1
			if _G.CSR_ShopItemsBought then
				shop_items = _G.CSR_ShopItemsBought[pid] or 0
			end
	...
	function CrimeSpreeManager:_get_modifiers(table_name, max_count, add_repeating)
		...
		if table_name == "forced" then
			-- Client in multiplayer: host handles forced modifiers, client receives via sync
			-- Return empty so popup auto-closes via managers.menu:back(true)
			...
			local repeating = tweak_data.crime_spree.repeating_modifiers
			...
			if _G.CSR_IsModifierRedundant and tweak_data.crime_spree.loud then
				...
				for i = #forced_mods, 1, -1 do
					...
					if
						mod.class
						and not string.find(mod.id or "", "csr_", 1, true)
						and not string.find(mod.id or "", "player_", 1, true)
					then
		...
		-- Track forced modifier count (player items are in CSR_PlayerItems now)
		local forced_count = 0
		for _, mod in ipairs(self._global.modifiers or {}) do
			if not (mod.id and string.find(mod.id, "^player_")) then
		...
		elseif current_count > CSR_LastModifierCount then
			...
			-- Clean any player items that vanilla select_modifier() added to _global.modifiers
			...
			DelayedCalls:Add("CSR_SaveAfterModifierSelection", 0.5, function()
				...
				if not cs or not cs._global then
					return
				end
				...
				if seed and CSR_SaveSeed then
					...
					for _, mod in ipairs(cs._global.modifiers or {}) do
				...
				-- Broadcast own items to peers in MP
				...
				-- Save to session file for reconnect (if client in foreign run)
		...
		-- Seed the RNG deterministically so the same items always appear for the same CS state.
		-- CSR_CurrentSeed is loaded from the seed file on startup and survives suspend/rejoin.
		-- player_item_count and spree_level are persisted by the game itself.
		local cs_seed = _G.CSR_CurrentSeed or 0
		local peer_id = CSR_LocalPeerId and CSR_LocalPeerId() or 1
		...
		-- Group available modifiers by item type (registry-driven).
		-- Skip is_scrap entries — scrap items live in CSR_PlayerItems but
		-- must never appear in selection offers (no buff, no use except as
		-- printer fodder).
		local by_type = {}
		for _, item in ipairs(_registry) do
			if not item.is_scrap then
				by_type[item.type] = {}
			end
		end
		...
		-- Rarity weights for the weighted selection draw (registry-driven)
		local rarity_weights = {}
		for _, item in ipairs(_registry) do
			rarity_weights[item.type] = item.weight
		end
	...
	Hooks:PostHook(CrimeSpreeManager, "reset_crime_spree", "CSR_ResetForcedCacheOnReset", function(self)
		...
		CSR_LastModifierCount = 0
		CSR_ModifierCountInitialized = false
		-- Reset bonus drop count so the item formula doesn't confuse
		-- bonus drops from the previous run with milestone picks
		_G.CSR_BonusDropCount = 0
		_G.CSR_BonusDropChance = (_G.CSR_ItemConstants or {}).bonus_drop_min_chance or 0.01
		-- Same rationale for shop purchases: last run's buys must not carry
		-- into this run's milestone accounting.
		_G.CSR_ShopItemsBought = {}
	end)

	-- Override: replace vanilla forced modifiers popup with custom notification
	-- Vanilla would open CrimeSpreeForcedModifiersMenuComponent — we show CSRForcedModsNotification instead
	function CrimeSpreeManager:check_forced_modifiers()
		-- Client: host controls forced mods
		if _G.CSR_MP and CSR_MP.is_client and CSR_MP.is_client() then
			return
		end
		...
		-- Get forced modifiers list (filtered by CSR_LastShownForcedLevel internally)
		local forced_mods = self:get_forced_modifiers()
		if not forced_mods or #forced_mods == 0 then
			return
		end

		-- Auto-apply modifiers with duplicate protection (matches crimespree_forced_modifiers_auto.lua)
		local mods_to_add = {}
		for _, modifier in ipairs(forced_mods) do
		...
		-- Update CSR_LastShownForcedLevel to prevent re-showing
		local max_level = 0
		...
		if max_level > 0 then
			_G.CSR_LastShownForcedLevel = math.max(_G.CSR_LastShownForcedLevel or 0, max_level)
		end
		...
		-- Refresh the CS details panel so the new forced mods appear in the modifiers list.
		-- perform_update() only refreshes text; refresh_crime_spree_details_gui() recreates
		-- the whole component (including the modifiers list) from server_active_modifiers().
		DelayedCalls:Add("CSR_RefreshDetailsAfterForcedMods", 0.5, function()
		...
		-- Show custom popup instead of vanilla node
