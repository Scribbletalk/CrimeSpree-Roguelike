...
_G.CSR_SafeOverride(
	...
	"_get_current_damage",
	"Evidence Rounds",
	original_get_current_damage,
	function(self, dmg_mul)
...
-- Evidence Rounds: count bullets fired by local player each mission
_G.CSR_BulletsFiredToday = _G.CSR_BulletsFiredToday or 0

Hooks:PostHook(RaycastWeaponBase, "fire", "CSR_EvidenceRoundsBulletCounter", function(self)
	if not managers.crime_spree or not managers.crime_spree:is_active() then
		return
	end
	...
	if owner == player_unit then
		_G.CSR_BulletsFiredToday = _G.CSR_BulletsFiredToday + 1
	end
end)
