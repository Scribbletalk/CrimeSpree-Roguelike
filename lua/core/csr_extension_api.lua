...
-- lua/core/csr_item_dispatch.lua calls on_script_loaded on every load). Kept here,
...
-- Auto-load CSR's own item definition files. Each lua/item_defs/*.lua CALLS
...
local function load_item_defs()
	if not (file and file.GetFiles and ModPath) then
		log("[CSR][api] item_defs: file API or ModPath unavailable -- skipped")
		return
	end
	local dir = ModPath .. "lua/item_defs/"
	...
	for _, name in pairs(names) do
		if type(name) == "string" and name:sub(-4) == ".lua" then
			...
			if ok then
				...
				log("[CSR][api] item_defs: '" .. tostring(name) .. "' failed to load: " .. tostring(err))
			end
		end
	end
	log("[CSR][api] item_defs: ran " .. count .. " item file(s)")
end
